...
namespace Dominio
{
    public class Sistema
    {
        ...
        private List<Seleccion> _selecciones { get; set; }
        private List<Pais> _paises { get; set; }
        private List<Partido> _partidos { get; set; }
        private List<Reseña> _resenias { get; set; }
        ...
        public Sistema()
        {
            ...
            _resenias = new List<Reseña>();
            PrecargaUsuarios();
            PrecargaPaises();
            PrecargaJugadores();
            PrecargaSelecciones();
            PrecargaFaseGrupos();
            PrecargaEliminatoria();
            PrecargaReseña();
        }
        //Singleton
        ...
        //GET ELIMINATORIAS
        public List<FaseEliminatoria> GetEliminatorias()
        {
            ...
            foreach (Partido p in _partidos)
            {
                ...
                if (p is FaseEliminatoria)
                {
                    ret.Add((FaseEliminatoria)p);
                }
            }
            return ret;
        }
        ...
        //GET FASEGRUPO
        public List<FaseGrupo> GetFaseGrupo()
        {

            List<FaseGrupo> ret = new List<FaseGrupo>();
        ...
        public List<Partido> GetPartidosJugador(int id)
        {
            List<Partido> _partidosDadoJugador = new List<Partido>();

            foreach (Partido p in _partidos)
            {
                if (p.SeleccionUno.EncontrarJugador(id) || p.SeleccionDos.EncontrarJugador(id))
                {
                    _partidosDadoJugador.Add(p);
                }
            }
            return _partidosDadoJugador;
        }

        public List<Jugador> GetJugadoresExpulsados()
        {
            List<Jugador> ret = new List<Jugador>();
        ...
        public List<Partido> GetPartidoConMasGoles(string seleccion) {
            ...
            foreach (Partido p in _partidos) {
                if (p.SeleccionUno.Pais.Nombre == seleccion || p.SeleccionDos.Pais.Nombre == seleccion)
                {
                    if (p.ContadorGolesPartido() > max)
                    {
                        max = p.ContadorGolesPartido();
                        ret.Clear();
                        ret.Add(p);
                    }
                    else if (p.ContadorGolesPartido() == max)
                    {
                        ret.Add(p);
                    }
                }
        ...
        public List<Jugador> GetJugadoresConGol()
        {
            List<Jugador> ret = new List<Jugador>();
        ...
        public List<Seleccion> GetSeleccionesOrdenadas()
        {
        ...
        public List<Jugador> GetJugadoresDe(int id)
        {
            List<Jugador> ret = new List<Jugador>();
            foreach (Jugador j in _jugadores)
            {
        ...
        public List<Partido> GetPartidosCerrados()
        {
            List<Partido> ret = new List<Partido>();
            foreach (Partido p in _partidos)
            {
        ...
        public List<Reseña> GetReseñasRealizo(int? id)
        {
            List<Reseña> ret = new List<Reseña>();
            foreach (Reseña r in _resenias) {
                if (r.Periodista.Id == id)
                {
                    ret.Add(r);
                }
            }
        ...
        {
            foreach (Periodista p in _usuarios)
            {
                if (p.Id == id)
                {
                    return p;
        ...
        public Periodista GetPeriodistaEmail(string email)
        {
            foreach (Periodista p in _usuarios)
            {
                if (p.Email == email)
        ...
        public List<Incidencia> GetIncidenciasDado(int id, int idP)
        {
            List<Incidencia> ret = new List<Incidencia>();
            foreach (Partido p in _partidos)
            {
                foreach (Incidencia i in p.GetIncidencias())
                {
                    if (p.Id == id & i.Jugador.Pais.Id == idP)
                    {
                        ret.Add(i);
                    }
                }
        ...
        public List<Usuario> GetPeriodistasOrdenados()
        {
        ...
        public List<Reseña> GetReseñasPeriodistaDado(int id)
        {
            List<Reseña> ret = new List<Reseña>();
            foreach (Reseña r in _resenias)
            {
        ...
        public Seleccion GetSeleccionDado(int id)
        {
            foreach (Seleccion s in _selecciones)
            {
                if (s.Pais.Id == id)
        ...
        public List<Partido> GetPartidoEntreFechas(DateTime fecha1, DateTime fecha2)
        {
            List<Partido> ret = new List<Partido>();
            foreach (Partido p in _partidos)
            {
                if (p.Finalizado)
                {
                    if (p.FechaHora > fecha1 && p.FechaHora < fecha2)
                    {
                        ret.Add(p);
                    } else if (p.FechaHora > fecha2 && p.FechaHora < fecha1)
                    {
                        ret.Add(p);
                    }
                }
        ...
        public List<Reseña> GetReseñasConRoja(string email)
        {
            List<Reseña> ret = new List<Reseña>();
            foreach (Reseña r in _resenias)
            {
                foreach (Incidencia i in r.Partido.GetIncidencias())
                {
                    if (r.Partido.Finalizado & r.Periodista.Email == email & i.Tipo == "Roja")
                    {
                        ret.Add(r);
                    }
                }
        ...
        public List<Seleccion> GetSeleccionesGoleadoras()
        {
            ...
            foreach (Seleccion s in _selecciones)
            {
                ...
                foreach (Partido p in _partidos)
                {
                    foreach (Incidencia i in p.GetIncidencias())
                    {
                        if (i.Tipo == "Gol" & s.Pais.Id == i.Jugador.Pais.Id)
                        {
                            contador++;
                        }
                    }
        ...
        public void PrecargaFaseGrupos()
        {
            try
            {
                Partido GrupoHP1 = new FaseGrupo("H", GetSeleccion("Uruguay"), GetSeleccion("Corea del Sur"), new DateTime(2022, 11, 24, 10, 00, 00));
                AltaPartido(GrupoHP1);
                GrupoHP1.Agregarincidencia(new Incidencia("Gol", 20, GetJugador("Gue-sung Cho")));
                GrupoHP1.Agregarincidencia(new Incidencia("Amarilla", 25, GetJugador("Diego Godín")));
                GrupoHP1.Agregarincidencia(new Incidencia("Gol", 54, GetJugador("Luis Suárez")));
                GrupoHP1.Agregarincidencia(new Incidencia("Roja", 74, GetJugador("Kyung-won Kwon")));
                GrupoHP1.Agregarincidencia(new Incidencia("Gol", 80, GetJugador("José María Giménez")));
                GrupoHP1.CierrePartido();

                Partido GrupoHP2 = new FaseGrupo("H", GetSeleccion("Portugal"), GetSeleccion("Ghana"), new DateTime(2022, 11, 24, 13, 00, 00));
                AltaPartido(GrupoHP2);
                GrupoHP2.Agregarincidencia(new Incidencia("Gol", 36, GetJugador("Bruno Fernandes")));
                GrupoHP2.Agregarincidencia(new Incidencia("Gol", 66, GetJugador("Antoine Semenyo")));

                Partido GrupoHP3 = new FaseGrupo("H", GetSeleccion("Ghana"), GetSeleccion("Corea del Sur"), new DateTime(2022, 11, 28, 10, 00, 00));
                AltaPartido(GrupoHP3);
                GrupoHP3.Agregarincidencia(new Incidencia("Gol", 15, GetJugador("Antoine Semenyo")));

                Partido GrupoHP4 = new FaseGrupo("H", GetSeleccion("Portugal"), GetSeleccion("Uruguay"), new DateTime(2022, 11, 28, 16, 00, 00));
                AltaPartido(GrupoHP4);
                GrupoHP4.Agregarincidencia(new Incidencia("Gol", 44, GetJugador("Bernardo Silva")));

                Partido GrupoHP5 = new FaseGrupo("H", GetSeleccion("Portugal"), GetSeleccion("Corea del Sur"), new DateTime(2022, 12, 02, 12, 00, 00));
                AltaPartido(GrupoHP5);
                GrupoHP5.Agregarincidencia(new Incidencia("Gol", 23, GetJugador("Gue-sung Cho")));
                GrupoHP5.Agregarincidencia(new Incidencia("Gol", 44, GetJugador("Bernardo Silva")));
                GrupoHP5.Agregarincidencia(new Incidencia("Gol", 78, GetJugador("Rafael Leão")));

                Partido GrupoHP6 = new FaseGrupo("H", GetSeleccion("Uruguay"), GetSeleccion("Ghana"), new DateTime(2022, 12, 02, 12, 00, 00));
                AltaPartido(GrupoHP6);
                GrupoHP6.Agregarincidencia(new Incidencia("Gol", 54, GetJugador("Federico Valverde")));

                Partido GrupoEP1 = new FaseGrupo("E", GetSeleccion("Alemania"), GetSeleccion("Japón"), new DateTime(2022, 11, 23, 10, 00, 00));
                AltaPartido(GrupoEP1);
                GrupoEP1.Agregarincidencia(new Incidencia("Gol", 56, GetJugador("Leon Goretzka")));
                GrupoEP1.Agregarincidencia(new Incidencia("Gol", 80, GetJugador("Leon Goretzka")));

                Partido GrupoEP2 = new FaseGrupo("E", GetSeleccion("España"), GetSeleccion("Costa Rica"), new DateTime(2022, 11, 23, 13, 00, 00));
                AltaPartido(GrupoEP2);
                GrupoEP2.Agregarincidencia(new Incidencia("Gol", 10, GetJugador("Álvaro Morata")));
                GrupoEP2.Agregarincidencia(new Incidencia("Gol", 25, GetJugador("Anthony Contreras")));
                GrupoEP2.Agregarincidencia(new Incidencia("Gol", 55, GetJugador("Raúl de Tomás")));

                Partido GrupoEP3 = new FaseGrupo("E", GetSeleccion("Costa Rica"), GetSeleccion("Japón"), new DateTime(2022, 11, 27, 07, 00, 00));
                AltaPartido(GrupoEP3);
                GrupoEP3.Agregarincidencia(new Incidencia("Gol", 69, GetJugador("Shuto Machino")));

                Partido GrupoEP4 = new FaseGrupo("E", GetSeleccion("España"), GetSeleccion("Alemania"), new DateTime(2022, 11, 27, 16, 00, 00));
                AltaPartido(GrupoEP4);
                GrupoEP4.Agregarincidencia(new Incidencia("Gol", 6, GetJugador("Raúl de Tomás")));
                GrupoEP4.Agregarincidencia(new Incidencia("Gol", 21, GetJugador("Timo Werner")));
                GrupoEP4.Agregarincidencia(new Incidencia("Gol", 44, GetJugador("Timo Werner")));
                GrupoEP4.Agregarincidencia(new Incidencia("Gol", 79, GetJugador("Raúl de Tomás")));

                Partido GrupoEP5 = new FaseGrupo("E", GetSeleccion("España"), GetSeleccion("Japón"), new DateTime(2022, 12, 01, 16, 00, 00));
                AltaPartido(GrupoEP5);
                GrupoEP5.Agregarincidencia(new Incidencia("Gol", 15, GetJugador("Raúl de Tomás")));

                Partido GrupoEP6 = new FaseGrupo("E", GetSeleccion("Costa Rica"), GetSeleccion("Alemania"), new DateTime(2022, 12, 01, 16, 00, 00));
                AltaPartido(GrupoEP6);
                GrupoEP6.Agregarincidencia(new Incidencia("Gol", 55, GetJugador("Thomas Müller")));
                GrupoEP6.Agregarincidencia(new Incidencia("Gol", 66, GetJugador("Keysher Fuller")));
                GrupoEP6.CierrePartido();
            }
            catch (Exception e)
            {
                throw new Exception(e.Message);
        ...
        public void PrecargaEliminatoria()
        {
            try
            {
                Partido E1 = new FaseEliminatoria(true, true, "Octavos", GetSeleccion("Uruguay"), GetSeleccion("España"), new DateTime(2022, 12, 16, 10, 00, 00));
                AltaPartido(E1);
                E1.Agregarincidencia(new Incidencia("Gol", 68, GetJugador("Álvaro Morata")));
                E1.Agregarincidencia(new Incidencia("Gol", 113, GetJugador("Federico Valverde")));
                E1.Agregarincidencia(new Incidencia("Gol", -1, GetJugador("Federico Valverde")));
                E1.Agregarincidencia(new Incidencia("Gol", -1, GetJugador("Álvaro Morata")));
                E1.Agregarincidencia(new Incidencia("Gol", -1, GetJugador("Luis Suárez")));
                E1.Agregarincidencia(new Incidencia("Gol", -1, GetJugador("Raúl de Tomás")));
                E1.Agregarincidencia(new Incidencia("Gol", -1, GetJugador("Diego Godín")));
                E1.CierrePartido();

                Partido E2 = new FaseEliminatoria(false, false, "Octavos", GetSeleccion("Portugal"), GetSeleccion("Alemania"), new DateTime(2022, 12, 18, 12, 00, 00));
                AltaPartido(E2);
                E2.Agregarincidencia(new Incidencia("Gol", 8, GetJugador("Thomas Müller")));
                E2.Agregarincidencia(new Incidencia("Gol", 15, GetJugador("Bruno Fernandes")));
                E2.Agregarincidencia(new Incidencia("Gol", 43, GetJugador("Bernardo Silva")));
                E2.Agregarincidencia(new Incidencia("Gol", 62, GetJugador("Leon Goretzka")));
                E2.Agregarincidencia(new Incidencia("Gol", 80, GetJugador("Thomas Müller")));
            }
            catch (Exception e)
            {
                throw new Exception(e.Message);
        ...
        {
            AltaUsuario(new Periodista("Fernando", "Niembro", "Fernandoniembro47@hotmail.es", "71247ESPN"));
            AltaUsuario(new Periodista("Pedro", "Niembro", "Pedrito@hotmail.es", "ESPN12345"));
            AltaUsuario(new Periodista("Renata", "Cruz", "prueba@gmail.es", "Pru12345"));
            AltaUsuario(new Periodista("Camilo", "Perez", "Perez@gmail.es", "ESPN12345"));
            AltaUsuario(new Periodista("Santiago", "Gonzalez", "cami13@gmail.es", "Pru12345"));
            AltaUsuario(new Operador(new DateTime(2020, 11, 15), "Emanuelle", "Gabriele", "Ema@gmail.com", "Ema12345"));
            AltaUsuario(new Operador(new DateTime(2020, 11, 15), "Matias", "Fernandez", "Matias@gmail.com", "Mati12345"));
        }

        public void PrecargaReseña()
        {
            AltaReseña(new Reseña(GetPeriodistaEmail("Fernandoniembro47@hotmail.es"), new DateTime(2022, 12, 01, 10, 00, 00), GetPartido(0), "Primer Partido", "Gran enfrentamiento entre Uruguay y Corea"));
